...
import com.ptoles.popularmovies.model.MovieTrailer;
...
class JsonDownloader extends AsyncTaskLoader<List<MoviePoster>> {
    ...
    private static List<MovieReview> movieReviews;
    private static List<MovieTrailer> movieTrailers;
    ...
    private JsonDownloader(Context context, String jsonData) {
        ...
        String thisMovieId = moviePosters.get(thisPoster).getMovieId();
           Log.i(TAG, moviePosters.get(thisPoster).toString());
          // movieReviews = JsonDownloader.DownloadReviewData(thisMovieId);
    ...
    public static List<MoviePoster> DownloadMovieData(String jsonURL){
       ...
       try {
           ...
           if(connection.getResponseCode()== HTTP_OK) {
               ...
               // Extract relevant fields from the JSON response and create a list of {@link MoviePosters}
    ...
    }//end - DownloadMovieData()


    public static List<MovieReview> DownloadReviewData(String jsonURL){
        ...
        try {
            ...
            if(connection.getResponseCode()== HTTP_OK) {
                ...
                // Extract relevant fields from the JSON response and create a list of {@link MoviePosters}
                movieReviews = JsonParser.parseReviewsFromJson(output.toString());
